...
Everything lives in a single `docker-compose.yml`. The default `docker compose up` starts the core cluster — Oxia, MinIO, three brokers and the Ursa compactor — which needs the locally built image. The Iceberg catalog sits behind the `lakehouse` profile, and every demo workload behind a profile of its own, so none of them start on their own.
...
One image is built locally: `lakestream/kafka` carries the brokers, the CLI tools and the standalone Ursa compactor (`/opt/kafka/bin/ursa-compactor.sh`, resolved from Maven Central at the ursa-storage version the tarball ships). Everything else is pulled from a public registry at a pinned version.

```bash
cd docker/examples/docker-compose-files/cluster/ursa

# -> lakestream/kafka:latest
./build-image.sh
# or: make build-image
```
...
| `make build-image` | Build the Kafka image (brokers, CLI and Ursa compactor) |
...
|----------|---------|-------------|
| `IMAGE` | `lakestream/kafka:latest` | Kafka broker, CLI and compactor image |
...
It is part of the default stack and runs from the same `lakestream/kafka` image as the brokers. Make sure the locally built image exists:
